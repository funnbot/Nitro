...
exports.run = (message, bot, suffix, args) => {
    if (!message.mentions.users.first()) return message.channel.send("You must mention a user to make them the host.");
    let can = music.checkHostPerm(message);
    if (can === "nm") return message.channel.send("There are no music sessions active.");
    if (can) {
        let user = message.mentions.users.first();
        if (!message.guild.member(user).voiceChannel) return message.channel.send("The new host must be in a voice channel.");
        music.setHost(message.guild.id, user.id, (err, did) => {
            if (err) return message.channel.send("There is no music session.");
            message.channel.send(user.username+" has been made the host.")
        })
    } else {
