const config = require('../functions/profile');
...
exports.run = (message, bot, suffix, args, send) => {
    if (!args[0]) return send('**:confused: | Provide a bet**');
    let id = message.author.id;
    let money = config.getMoney(id);
    let bet = parseInt(args[0]) || "no";
    ...
    send(slot)
    if (earned) config.setMoney(id, money+earned)
    else config.setMoney(id, money-bet);
}
