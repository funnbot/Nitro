...
function variables(text, message, bot) {
    const repl = {
        ...
        "{guild.bots}":Array.from(message.guild.members).filter(bot => bot[1].user.bot === true).map(ar => ar[1].username).join(", "),
        "{guild.bots.size}":message.guild.members.filter(bot => bot.user.bot === true).size,
        "{guild.users}":Array.from(message.guild.members).filter(bot => bot[1].user.bot === false).map(ar => ar[1].username).join(", "),
        "{guild.users.size}":message.guild.members.filter(bot => bot.user.bot === false).size,
