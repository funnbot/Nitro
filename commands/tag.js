exports.run = (message, bot) => {
  ...
  if (message.args[0] === "add") {
    if (!message.args[1]) return message.channel.send(`To add a tag use:\n\`${prefix}tag add <name> <the tags text>\``);
    if (tags[message.args[1]]) return message.channel.send("**The tag `" + message.args[1] + "` already exists**");
    if (!message.args[2]) return message.channel.send(`Provide the text for the tag after the name.`);
    ...
    message.channel.send("**Adding the tag `" + message.args[1] + "`**.")
    return bot.config.setTags(message.guild.id, tags)

  } else if (message.args[0] === "delete") {
    if (!message.args[1]) return message.channel.send(`To delete a tag use:\n\`${prefix}tag delete <tagname>\``);
    if (!tags[message.args[1]]) return message.channel.send("**The tag `" + message.args[1] + "` does not exist.**");
    ...
    if (message.author.id === tag.owner || message.channel.permissionsFor(message.member).hasPermission("MANAGE_GUILD")) {
      ...
      return message.channel.send("**The tag `" + message.args[1] + "` has been deleted**");
    } else {
